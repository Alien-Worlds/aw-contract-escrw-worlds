...
The `@alien-worlds/escrw-worlds-common` package is a comprehensive set of components designed to seamlessly integrate blockchain contracts into our projects. This package serves as a foundational element, providing essential functionalities for interacting with blockchain contracts and retrieving data.
...
The code is based on the ABI of the contract, the list of included actions and tables can be found here [https://wax.bloks.io/account/escrw.worlds](https://wax.bloks.io/account/escrw.worlds)
...
- **Data Sources**: Data sources `EscrwWorldsActionMongoSource` and `EscrwWorldsDeltaMongoSource`, which enable access to the underlying mongodb action and delta collections. Data source is an integral part of the repositories.

- **Repositories**: Repositories `EscrwWorldsActionRepository` and `EscrwWorldsDeltaRepository` that facilitate CRUD operations for actions and deltas stored in the database.
...
To start utilizing the `@alien-worlds/escrw-worlds-common` package, follow these steps:

1. Install the package by running:

```
yarn add @alien-worlds/escrw-worlds-common
```

2. Import the necessary components into your project:

```typescript
import * as EscrwWorldsContract from '@alien-worlds/escrw-worlds-common';
```
